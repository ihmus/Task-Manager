...
import pytz
from datetime import datetime
from flask import Blueprint, render_template, request, flash, jsonify, redirect, url_for, abort, current_app, send_from_directory
from flask_login import login_required, current_user
from sqlalchemy.orm import joinedload
from sqlalchemy import text
from werkzeug.utils import secure_filename

from .models import Note, User, Attachment, Category
from .utils import role_required
from . import db
...
def gorevler():
    ...
    for note in notes:
        ...
        seconds = int((now - note_date).total_seconds())
        if seconds < 60:
            note.time_passed, note.color = f"{seconds} sn önce", "green"
        elif seconds < 3600:
            note.time_passed, note.color = f"{seconds // 60} dk önce", "orange"
        elif seconds < 86400:
            note.time_passed, note.color = f"{seconds // 3600} saat önce", "red"
        else:
            note.time_passed, note.color = f"{seconds // 86400} gün önce", "brown"

    categories = Category.query.all() if current_user.role == "admin" else \
                 Category.query.filter(Category.notes.any(user_id=current_user.id)).all()

    return render_template("gorevler.html", notes=notes, all_notes=all_notes, 
                           default_mode=default_mode, categories=categories, 
                           selected_category_id=category_id, active_page='gorevler')
...
def create_note():
    ...
    try:
        ...
        new_note = Note(
            ...
            start_date=datetime.strptime(request.form.get("start_date"), "%Y-%m-%dT%H:%M").replace(tzinfo=pytz.utc) if request.form.get("start_date") else None,
            deadline=datetime.strptime(request.form.get("deadline"), "%Y-%m-%dT%H:%M").replace(tzinfo=pytz.utc) if request.form.get("deadline") else None,
            duration_days=int(request.form.get("duration")) if request.form.get("duration") else None
        )
        db.session.add(new_note)
        db.session.flush()
        ...
        if uploaded and uploaded.filename and allowed_file(uploaded.filename):
            ext = secure_filename(uploaded.filename).rsplit(".", 1)[1].lower()
            stored_name = f"{uuid.uuid4().hex}.{ext}"
            upload_folder = current_app.config.get("UPLOAD_FOLDER", "uploads")
            os.makedirs(upload_folder, exist_ok=True)
            uploaded.save(os.path.join(upload_folder, stored_name))
            
            att = Attachment(filename=secure_filename(uploaded.filename), stored_name=stored_name, 
                             mime_type=uploaded.mimetype, size=0, note_id=new_note.id)
            db.session.add(att)
...
# --- ADMİN PANELİ VE KULLANICI YÖNETİMİ ---
...
def admin_panel():
    page = request.args.get('page', 1, type=int)
    users_pagination = User.query.options(joinedload(User.notes)).paginate(page=page, per_page=12, error_out=False)
    user_cards = []
    for user in users_pagination.items:
        notes = sorted(user.notes, key=lambda n: n.date, reverse=True)
        user_cards.append({
            'id': user.id, 'name': user.first_name or user.email, 'role': user.role,
            'total_notes': len(notes), 'completed': sum(1 for n in notes if n.completed),
            'recent_notes': notes[:3]
        })
    return render_template('admin.html', users=users_pagination, user_cards=user_cards, active_page='admin_panel')
...
@login_required
def users_list():
    users_pagination = User.query.paginate(page=request.args.get('page', 1, type=int), per_page=12)
    user_cards = []
    for user in users_pagination.items:
        if current_user.role != "admin" and user.role == "admin": continue
        user_cards.append({
            'id': user.id, 'name': user.first_name or user.email, 'role': user.role,
            'total_notes': len(user.notes), 'completed': sum(1 for n in user.notes if n.completed),
            'recent_notes': sorted(user.notes, key=lambda n: n.date, reverse=True)[:3]
        })
    return render_template('users_list.html', users=users_pagination, user_cards=user_cards, active_page='pano')

@views.route("/user/<int:user_id>")
@login_required
def user_profile(user_id):
    user = User.query.get_or_404(user_id)
    if current_user.role != 'admin' and current_user.id != user.id and user.role == 'admin': abort(403)
    notes = Note.query.filter_by(user_id=user.id).order_by(Note.date.desc()).all()
    return render_template("user_profile.html", user=user, recent_notes=notes[:10], total_tasks_count=len(notes))

# --- DOSYA VE DİĞER İŞLEMLER ---

@views.route('/download/attachment/<int:attachment_id>')
@login_required
def download_attachment(attachment_id):
    attachment = Attachment.query.get_or_404(attachment_id)
    if current_user.role != 'admin' and attachment.note.user_id != current_user.id: abort(403)
    return send_from_directory(current_app.config['UPLOAD_FOLDER'], attachment.stored_name, as_attachment=True, download_name=attachment.filename)
...
def toggle_note(note_id):
    ...
    note.completed = not note.completed
    db.session.commit()
    return redirect(url_for('views.gorevler', default_mode=request.form.get('default_mode', 1)))
...
def istatistikler():
    ...
    total = len(all_notes)
    completed = sum(1 for n in all_notes if n.completed)
    return render_template('istatistikler.html', total_tasks=total, completed_tasks=completed, active_page='istatistikler')
